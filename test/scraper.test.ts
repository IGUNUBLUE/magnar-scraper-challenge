...
import { extractDocuments, initialSearchRequest, paginationRequest, panelFromPartialResponse, resultPageCount } from "../src/parser.js";
...
test("builds the escaped JSF general-search command and reads the full page count", () => {
  const landing = `<form id="formBuscador" action="/resultado.xhtml"><input type="hidden" name="javax.faces.ViewState" value="state"/><input type="image" onclick="mojarra.jsfcljs(document.getElementById(\\'formBuscador\\'),{\\'formBuscador:j_idt69\\':\\'formBuscador:j_idt69\\',\\'forward\\':\\'buscar\\'},\\'\\')"/></form>`;
  const request = initialSearchRequest(landing, "https://example.test/inicio.xhtml", "civil");
  assert.equal(request.url, "https://example.test/resultado.xhtml");
  assert.equal(request.body.get("forward"), "buscar");
  assert.equal(request.body.get("formBuscador:txtBusqueda"), "civil");
  assert.equal(resultPageCount("<script>maxValue: 20964</script>"), 20964);
  assert.throws(() => resultPageCount("<html></html>"), /total number/);
});
...
test("records a PDF that remains rate-limited", async () => {
  const directory = await mkdtemp(join(tmpdir(), "magnar-scraper-"));
  const transport: HttpTransport = {
    async get(): Promise<HttpResult<string | Uint8Array>> { return { status: 429, headers: { "retry-after": "0" }, data: new Uint8Array() }; },
    async post(): Promise<HttpResult<string>> { throw new Error("not used"); },
  };
  const downloader = new PdfDownloader(transport, directory, join(directory, "failed.ndjson"), 2);
  const result = await downloader.download({ id: "retry-me", page: 1, fields: {}, sourceUrl: "https://example.test", pdfUrl: "https://example.test/retry.pdf" });
  assert.equal(result, "failed");
  const failure = JSON.parse(await readFile(join(directory, "failed.ndjson"), "utf8"));
  assert.equal(failure.documentId, "retry-me");
